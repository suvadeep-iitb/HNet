...
CLIP=0.25
MIN_LR_RATIO=0.004
INPUT_LENGTH=123000 # Trace length or size of the attack window.
                    # Note that since the traces has been compressed by
		    # a factor of 2, the trace length of the compressed
		    # traces becomes 125000. We set it to 123000
		    # to introduces desync in the traces
...
SEG_LEN=2500 # Segment length; since the traces has been compressed by a factor of 2, 
	     # setting segment length to 2500 makes the effective segment length 5000
SEG_STRIDE=1500  # Stride length for creating new segment; overlap between two consequetive
		 # segments is $SEG_LEN-$SEG_STRIDE. Since the traces has been compressed by a 
		 # factor of 2, setting stride length to 1500 makes the effective stride
		 # length 3000. Therefore, the effective overlap between two consequetive
		 # segments is 5000-3000=2000
SM_ATTN=True
