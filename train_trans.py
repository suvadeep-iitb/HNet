...
from transformer import HierTransformer as Transformer
...
flags.DEFINE_integer("start_idx", default=0,
      help="The start index of the attack window")
flags.DEFINE_integer("prof_desync", default=0,
      help="The profiling desync")
flags.DEFINE_integer("attack_desync", default=0,
      help="The attack desync")
...
flags.DEFINE_integer("n_head_softmax", default=4,
      help="Number of attention heads in softmax attention of internal EstraNet model")
flags.DEFINE_integer("d_head_softmax", default=32,
      help="Dimension of each head in softmax attention of internal EstraNet model")
...
flags.DEFINE_string("head_initialization", default='forward',
      help="Type of the initialization of the positional attention heads of internal EstraNet model, can be in ['forward', 'backward', 'symmetric']")
flags.DEFINE_integer("seg_len", default=5000,
      help="Length of each segment at the first layer of the HierNet")
flags.DEFINE_integer("seg_stride", default=3000,
      help="Stride of each segment at the first layer of the HierNet")
flags.DEFINE_bool("softmax_attn", default='True',
      help="Whether to use softmax attention instead of global pooling in internal EstraNet model")
...
def create_model(n_classes):
    model = Transformer(
        ...
        seg_stride = FLAGS.seg_stride,
        input_len = FLAGS.input_length,
...
def train(train_dataset, test_dataset, num_train_batch, num_eval_batch, strategy, chk_name):
    ...
    # Calculate the maximum number of batches supporated by tensorflow PROTOBUF
    max_supported_num_train_batches = train_dataset.GetMaxSupportedNumBatches(
                                            FLAGS.train_batch_size
                                      )
    max_supported_num_eval_batches = test_dataset.GetMaxSupportedNumBatches(
                                            FLAGS.eval_batch_size
                                      )

    ##### Create computational graph for a smaller sample of train and test
    ##### datasets for evaluation
    num_sampled_eval_batch = min(num_eval_batch//10, max_supported_num_eval_batches)
    eval_train_dataset = train_dataset.GetSampledDataset(
                            num_sampled_eval_batch, 
                            FLAGS.eval_batch_size, 
                            training=True
                         )
    eval_dist_train_dataset = strategy.experimental_distribute_dataset(
                                  eval_train_dataset
                              )

    eval_test_dataset = test_dataset.GetSampledDataset(
                            num_sampled_eval_batch, 
                            FLAGS.eval_batch_size, 
                            training=True
                        )
    eval_dist_test_dataset = strategy.experimental_distribute_dataset(
                                 eval_test_dataset
                             )
    ...
    def train_steps(iterator, steps, bsz):
        ...
        def step_fn(inps, lbls):
            ...
            with tf.GradientTape() as tape:
                logits = model(inps, 1.0, training=True)[0]
                if FLAGS.dataset == 'CHES20':
    ...
    while cur_step < FLAGS.train_steps:
        # The whole train data may not fit into tensorflow protobuf
        # Repeatedly create small sampled dataset and train model on it
        for start_iter_batch in range(0, FLAGS.iterations, max_supported_num_train_batches):
            iter_batch = min(max_supported_num_train_batches, FLAGS.iterations-start_iter_batch)
            sampled_train_dataset = train_dataset.GetSampledDataset(
                                        iter_batch, FLAGS.train_batch_size, training=True
                                    )
            sampled_train_dist_dataset = strategy.experimental_distribute_dataset(
                                            sampled_train_dataset
                                         )
            train_steps(iter(sampled_train_dist_dataset), 
                        tf.convert_to_tensor(iter_batch), 
                        FLAGS.train_batch_size
                       )
            del sampled_train_dataset, sampled_train_dist_dataset
        ...
        if FLAGS.max_eval_batch <= 0:
            num_eval_iters = num_sampled_eval_batch
        else:
            num_eval_iters = min(FLAGS.max_eval_batch, num_sampled_eval_batch)

        eval_loss.reset_states()
        eval_steps(iter(eval_dist_train_dataset), 
                   tf.convert_to_tensor(num_eval_iters),
                   FLAGS.eval_batch_size
                  )
        ...
        eval_loss.reset_states()
        eval_steps(iter(eval_dist_test_dataset), 
                   tf.convert_to_tensor(num_eval_iters),
                   FLAGS.eval_batch_size
                  )
...
def evaluate(data, num_batches, strategy, chk_name):
    ...
    if FLAGS.output_attn:
        dataset = data.GetSampledDataset(num_batches, FLAGS.eval_batch_size, training=False)
        return model.predict(dataset)

    max_supported_num_batches = data.GetMaxSupportedNumBatches(FLAGS.eval_batch_size)
    output = []
    # The whole evaluation data may not fit into tensorflow protobuf.
    # Repeatedly create small sample and call predict on it
    for start_iter_batch in range(0, num_batches, max_supported_num_batches):
        num_iter_batch = min(max_supported_num_batches, num_batches-start_iter_batch)
        dataset = data.GetSampledDataset(num_iter_batch, FLAGS.eval_batch_size, training=False)
...
def print_hyperparams():
    ...
    tf.compat.v1.logging.info("start_idx             : %s" % (FLAGS.start_idx))
    tf.compat.v1.logging.info("prof_desync           : %s" % (FLAGS.prof_desync))
    tf.compat.v1.logging.info("attack_desync         : %s" % (FLAGS.attack_desync))
    ...
    tf.compat.v1.logging.info("seg_stride            : %s" % (FLAGS.seg_stride))
...
def main(unused_argv):
    ...
    if FLAGS.dataset == 'ASCAD':
        train_data = data_utils.Dataset(data_path=FLAGS.data_path, split="train", 
                ...
                start_idx=FLAGS.start_idx, desync=FLAGS.prof_desync, seed=0)
        test_data = data_utils.Dataset(data_path=FLAGS.data_path, split="test",
                input_length=FLAGS.input_length, data_desync=FLAGS.data_desync,
                start_idx=FLAGS.start_idx, desync=FLAGS.attack_desync, seed=1)
    ...
    if FLAGS.do_train:
        num_train_batch = train_data.num_all_samples // FLAGS.train_batch_size
        num_test_batch = test_data.num_all_samples // FLAGS.eval_batch_size
        ...
        train(train_data, test_data, num_train_batch, num_test_batch, strategy, chk_name)
    else:
        num_test_batch = test_data.num_all_samples // FLAGS.eval_batch_size
        if FLAGS.output_attn:
            num_test_batch = min(num_test_batch, FLAGS.max_eval_batch)

        tf.compat.v1.logging.info("num of test batches {}".format(num_test_batch))

        output = evaluate(test_data, num_test_batch, strategy, chk_name)
        ...
        if FLAGS.output_attn and not FLAGS.do_train:
            ...
            nsamples = test_data.num_all_samples
        if FLAGS.dataset == 'ASCAD':
